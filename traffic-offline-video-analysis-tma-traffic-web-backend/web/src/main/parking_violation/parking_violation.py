...
# Initialize logging
logging.info('Starting YoloV9 Parking Violation Script...')
...
def setup_zones(first_rectangle_points, cap):
    ...
    big_polygon = scale_points(first_rectangle_points)
...
def read_license_plate_from_image(license_plate_crop):
    enhanced_image = enhance_image(license_plate_crop)
    ...
    if detections_top is None or len(detections_top) == 0:
        print("No text detected in top half")
        top_text = ""
    else:
        # Ensure detections_top is iterable and check structure
        if isinstance(detections_top, list) and all(isinstance(line, list) for line in detections_top):
        ...
        format_top_text = format_top_license(cleaned_top_text)
        validated_top_text = validate_top_line(format_top_text)
        print(f"Top line detected text: {validated_top_text}")

    # Handle OCR detection for bottom half
    detections_bottom = reader.ocr(bottom_half)
    if detections_bottom is None or len(detections_bottom) == 0:
        print("No text detected in bottom half")
        bottom_text = ""
    else:
        # Ensure detections_bottom is iterable and check structure
        if isinstance(detections_bottom, list) and all(isinstance(line, list) for line in detections_bottom):
        ...
        format_bottom_text = format_bottom_license(cleaned_bottom_text)
        validated_bottom_text = validate_bottom_line(format_bottom_text)
        print(f"Bottom line detected text: {validated_bottom_text}")

    return validated_top_text, validated_bottom_text

def crop_license_plate(image, x1, y1, x2, y2, padding= 6):
    """
    Crop the license plate region from the image with optional padding.
 
    Args:
        image (numpy.ndarray): The input image from which to crop the license plate.
        x1, y1, x2, y2 (float): The bounding box coordinates.
        padding (int, optional): Padding around the bounding box to include extra space. Default is 10 pixels.
...
def callback(frame: np.ndarray, index: int, framecount, length, zone, byte_tracker, zone_annotator, box_annotator, trace_annotator, base_dir, start_time, video_name, selectedParkingViolation) -> np.ndarray:
    ...
    for tracker_id, class_id, xyxy in zip(detections.tracker_id, detections.class_id, detections.xyxy):
        ...
        if tracker_id not in first_detection_timestamps:
            first_detection_timestamps[tracker_id] = (framecount[0] / length) * (length / 30)
            first_detection_time[tracker_id] = (framecount[0] / length) * (length / 30)
        last_detection_timestamps[tracker_id] = (framecount[0] / length) * (length / 30)
        ...
        violationTimeLimit = 0

        if(selectedParkingViolation == "parking"):
            violationTimeLimit = 120
        else:
            violationTimeLimit = 10
        
        if(time_difference > violationTimeLimit):
...
def process_video(input_video_path, output_video_path, first_rectangle_points, selectedParkingViolation):
    ...
    zone = setup_zones(first_rectangle_points, cap)
    ...
    sv.process_video(
        ...
        callback=lambda frame, index: callback(frame, index, framecount, length, zone, byte_tracker, zone_annotator, box_annotator, trace_annotator, base_dir, start_time, video_name, selectedParkingViolation)
    )
...
if __name__ == "__main__":
    ...
    parser.add_argument('--selected_parking_violation', type=str, required=True, help="Parking Violation Type")
    ...
    process_video(args.input, args.output, first_rectangle_points, args.selected_parking_violation)
